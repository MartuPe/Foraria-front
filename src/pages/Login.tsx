...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const requires = res.requiresPasswordChange === true || localStorage.getItem("requiresPasswordChange") === "true";
      ...
      const target = role === Role.ADMIN || role === Role.CONSORCIO ? "/admin/dashboard" : "/dashboard";
      navigate(target);
    } catch (err) {
  ...
  return (
    <Box className="foraria-login-page foraria-wrapper">
      <Box className="foraria-form-container" component="form" onSubmit={handleSubmit}>
        ...
        <Typography variant="h5" component="h1" gutterBottom className="foraria-form-title">Iniciar Sesión</Typography>

        <TextField label="Email" fullWidth margin="normal" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="tu@email.com" InputLabelProps={{ shrink: true }} />
        <TextField
          label="Contraseña"
          fullWidth
          margin="normal"
        ...
        <Box className="foraria-centered-link" sx={{ mt: 2 }}>
          <Link component={RouterLink} to="/recuperar" underline="hover" className="foraria-form-link">Olvidé mi contraseña</Link>
        </Box>
      </Box>
    </Box>
  );
